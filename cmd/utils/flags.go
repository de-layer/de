...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --daozang instead)",
	...
	DaozangFlag = &cli.BoolFlag{
		Name:     "daozang",
		Usage:    "Daozang network: pre-configured test network for De Layer",
		Category: flags.EthCategory,
	}
	// Dev mode
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(DaozangFlag.Name) {
			return filepath.Join(path, "daozang")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(BootnodesFlag.Name) {
		...
		case ctx.Bool(DaozangFlag.Name):
			urls = params.DaozangBootnodes
		}
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(DaozangFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "daozang")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, GoerliFlag, SepoliaFlag, HoleskyFlag, DaozangFlag)
	...
	case ctx.Bool(DaozangFlag.Name):
		...
		cfg.Genesis = core.DefaultDaozangGenesisBlock()
		cfg.SyncMode = downloader.FullSync
		SetDNSDiscoveryDefaults(cfg, params.DaozangGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(DaozangFlag.Name):
		genesis = core.DefaultDaozangGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
